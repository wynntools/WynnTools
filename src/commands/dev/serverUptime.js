  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addRoleOption((option) =>
          option
            .setName('role')
            .setDescription('Assign a role to be pinged server changes state')
            .setRequired(false)
        )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('enable')
        .setDescription(
          'Enable the server uptime logging in your server (If you already have a config)'
        )
    )
    .addSubcommand((subcommand) =>
    ...
    try {
      ...
      if (subcommand === 'config') {
        ...
        if (serverUptimeConfig[interaction.guild.id]) {
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `This guild already has a config set for server uptime logging\nIts currently set to:\n**Channel:**<#${
                ...
                serverUptimeConfig[interaction.guildId].ghostPing
              }\n**Delete Previous Msgs:** ${serverUptimeConfig[interaction.guildId].deleteMsgs}\n${
          ...
          var overrideConfigMessage = await interaction.reply({
            embeds: [embed],
            components: [overrideRow],
          });
            ...
            if (confirmation.customId == 'serverUptimeSetupOverrideYes') {
              ...
              try {
                ...
                if (confirmOverrideConfirmation.customId == 'serverUptimeSetupConfirmOverrideYes') {
                  ...
                  confirmOverrideConfirmation.customId == 'serverUptimeSetupConfirmOverrideCancel'
                ) {
        ...
        if (serverUptimeConfig[interaction.guild.id]) {
          if (serverUptimeConfig[interaction.guild.id].disabled) {
            ...
            var disabledMessage = await interaction.reply({
              embeds: [embed],
            ...
            try {
              ...
              if (confirmation.customId == 'serverUptimeDeleteConfigYes') {
                ...
                fs.writeFileSync(
                  'data/serverUptime/config.json',
                  JSON.stringify(serverUptimeConfig)
                );
                const updatedEmbed = new EmbedBuilder()
                  .setColor(config.discord.embeds.green)
